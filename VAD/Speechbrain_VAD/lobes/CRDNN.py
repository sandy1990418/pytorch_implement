...
"""
import torch
import VAD.Speechbrain_VAD.nnet.linear as linear
import VAD.Speechbrain_VAD.nnet.normalization as normalization
import VAD.Speechbrain_VAD.nnet.pooling as pooling
import VAD.Speechbrain_VAD.nnet.dropout as dropout_module
import VAD.Speechbrain_VAD.nnet.RNN as RNN
import VAD.Speechbrain_VAD.nnet.containers as containers
import VAD.Speechbrain_VAD.nnet.CNN as CNN


class CRDNN(containers.Sequential):
    ...
    def __init__(
        ...
        if cnn_blocks > 0:
            self.append(containers.Sequential, layer_name="CNN")
        ...
        if projection_dim != -1:
            self.append(containers.Sequential, layer_name="projection")
            ...
            self.projection.append(
                normalization.LayerNorm, layer_name="norm"
            )
            self.projection.append(activation(), layer_name="act")

        if rnn_layers > 0:
            if use_rnnp:
                self.append(containers.Sequential, layer_name="RNN")
        ...
        if dnn_blocks > 0:
            self.append(containers.Sequential, layer_name="DNN")
...
class CNN_Block(containers.Sequential):
    ...
    def __init__(
        ...
        self.append(normalization.LayerNorm, layer_name="norm_1")
        ...
        self.append(normalization.LayerNorm, layer_name="norm_2")
        ...
        self.append(
            dropout_module.Dropout2d(drop_rate=dropout), layer_name="drop"
        )


class DNN_Block(containers.Sequential):
    ...
    def __init__(
        ...
        super().__init__(input_shape=input_shape)
        self.append(
            linear.Linear, n_neurons=neurons, layer_name="linear",
        )
        self.append(normalization.BatchNorm1d, layer_name="norm")
...
if __name__ == '__main__':
    ...
    waveform, sample_rate = torchaudio.load("./whisper_finetune/TedTalk/commonvoice_test_tw.wav")
    # waveform= waveform.view(1,-1,1)

    print(waveform.shape)
    model = CRDNN(input_size=waveform.view(-1).shape)
    # model = CRDNN([1, 3000, 1])
    checkpoint = torch.load('VAD/Speechbrain_VAD/vad-crdnn-libriparty/model.ckpt',map_location=torch.device('cpu'))
