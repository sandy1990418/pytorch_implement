...
import tqdm
import pathlib
import VAD.Speechbrain_VAD.utils.distributed as distributed
from numbers import Number
import VAD.Speechbrain_VAD.pretrained.fetching as fetch
...
def split_path(path):
    ...
    if isinstance(path, fetch.FetchSource):
        fetch_from, fetch_path = path
        source, filename = split(fetch_path)
        return fetch.FetchSource(fetch_from, source), filename
    else:
        return split(path)
